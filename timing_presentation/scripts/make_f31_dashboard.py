...
FIG_NUM = 31
RUN_ROOT = ROOT / "logs/visibility_comparison/f31_b1_route_choice_v1/F31_b1_apron_a3_mid"
...
def load_summary(run_dir: Path) -> dict:
    """Load run summary; accept interrupted runs — they still have trajectory data."""
    if run_dir is None:
        return {}
    ...
    # Patch interrupted runs so downstream code gets usable fields
    if d.get("completion_reason") == "interrupted" and "path_length_m" not in d:
        d["completion_reason"] = "interrupted (campaign timeout)"
        d.setdefault("crashed", False)
        d.setdefault("goal_reached", False)
    return d
...
def main() -> None:
    ...
    title = (
        ...
        f"warehouse_aws  |  local_horizon=10  v_max=0.60  |  global_nogo=0.20\n"
