#!/usr/bin/env python3
"""F25: Gazebo smoke diagnostic for F24 R01.

This is intentionally a diagnostic figure, not paper evidence. It compares the
first R01 Gazebo smoke for C1/C2 after the F24 offline battery looked promising.
"""
...
RUN_ROOT = ROOT / "logs/visibility_comparison/f24_r01_gazebo_smoke_v2/F24_R01_a4_lower_to_a3_mid"
WORLD_PROFILE = ROOT / "src/experiments/config/world_profiles.yaml"
OUT_DIR = ROOT / "timing_presentation/figures/F25"
...
def main() -> None:
    ...
    fig.suptitle(
        "F25 - R01 Gazebo smoke: offline-visible route does not yet survive closed-loop tracking",
        fontsize=15,
        fontweight="bold",
    )
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    png = OUT_DIR / "F25_r01_gazebo_smoke.png"
    pdf = OUT_DIR / "F25_r01_gazebo_smoke.pdf"
    fig.savefig(png, dpi=220)
    fig.savefig(pdf)

    md = OUT_DIR / "F25_r01_gazebo_smoke.md"
    md.write_text(
        "\n".join(
            [
