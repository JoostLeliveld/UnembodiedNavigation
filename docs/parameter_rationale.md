...
| Goal-prior schedule: 80 to 4, power 0.45, 90 steps | **Retain, label as objective tuning.** The widths are in the fixed camera cost chart, not metres. The schedule uses step progress; its physical duration depends on dt. These parameters cannot be defended as localization precision or a physical goal radius. |
