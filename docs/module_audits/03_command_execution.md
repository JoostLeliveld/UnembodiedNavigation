...
**2026-09-07 implementation follow-up:** [Remaining command/GLOBAL repairs, verification and unresolved integration gates](03_remaining_repairs_20260907.md). The ranked findings below describe the frozen audit baseline; this follow-up distinguishes subsequent code changes.
...
**Subsequent isolated integration evidence from investigation 14:** the original hypothesis above is now supported by a private Sim6/DART fixture using the installed ROS adapter, separate clock/command/odometry bridges, active default noise parameters and seed 210. Its [watchdog action/event record](../../experiments/runtime_integrity/simulator_world_20260906/watchdog_results.json) pins the adapter to the same `ce98aba1…` source hash. Killing/restarting that private adapter without fresh input left its output unchanged while native odometry and the [native physical-pose reference](../../experiments/runtime_integrity/simulator_world_20260906/watchdog_native_summary.json) continued moving; an explicit final zero stopped it. Suspending only the private clock bridge froze ROS time while native physics continued moving; restoring that bridge allowed watchdog zero. Ordinary receipt silence stopped, and reset without a new receipt also triggered zero. This upgrades physical persistence for those isolated adapter-death/restart and clock-loss conditions; it does not establish a failure in a selected navigation drive or invalidate the healthy-clock silence test. Investigation 03 did not perform these interventions, and its original 31 method cases remain unchanged.
