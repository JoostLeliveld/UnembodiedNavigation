...
**Current-source update, 2026-09-07:** [The ranked revalidation and repair handoff](02_current_source_handoff.md) verifies HEAD `f9e1a312` including repair packages A (`5e844a89`) and B, plus the recorded working-tree refactor. **19 deterministic cases reproduced; 90 focused regressions passed.** The current callback/writer/clock maps, stable source-line references, triggers, consequences, repair sequence and evidence are in that update.

Package A closes the odometry part of T02-01, mandatory-envelope startup T02-02, and outage snapshot race T02-05. Package B closes ordinary-stop cancellation and computation-age checks at LOCAL/direct tape installation. Do not implement these repairs again or treat the September 6 results below as current source.

Remaining verified findings include reverse-order belief publication, incomplete epoch validity, command fallback history order, same-time revisions discarded by the manager, mismatched planning metadata/frame, correction-worker starvation, and optional mission/pixel validity defects. The new probes also reproduce a post-commit diagnostic failure leaving no committed batch identity/terminal record, and a goal callback leaving the newest goal paired with an older progress signature. Current detector/manager reset guards and the mission's new unchanged-goal suppression are explicitly credited in the update. No production, Q/R, turn-observation, recovery-policy or live experiment changes were made by this audit.

## Historical baseline — 2026-09-06

**Status at the source hashes recorded below:** the recent single-clock and correction-writer repairs held, but publication order, input-history order and reset ownership remained incomplete. A slow belief prediction could replace a newer publication without any correction occurring. Delayed odometry callbacks could corrupt replay order. An in-place simulation reset could leave planning using the previous epoch's belief while reporting age zero.
...
Read first: `AGENTS.md`, `PLAN.md`, `docs/localization_metrics.md`, `docs/localization_metrics_registry.json`, `docs/open_questions.md`, and `docs/runtime_integrity_audit.md`. Also inspected [the existing estimation review](../state_estimation_module_review.md), [investigation map](../module_issue_investigations.md), the active configuration and launch wiring, and the named nodes/tests. [Audit 01](01_state_estimation.md) became available after this report's initial delivery and was read during the coordination follow-up. It independently confirms the overlapping input-order, coverage/replay and anonymous-bootstrap defects (R01–R03), and owns filter mathematics and recovery proposals. The earlier review's F01/F03/F05/F06/F07/F09/F10 findings remain relevant; they are not claimed as new discoveries here.
...
**Logging handoff:** coordinate the ledger with the logging investigation: retain capture, producer receipt/publication, assimilation receipt/commit, belief prediction target/publication, and logger receipt as separate fields with declared domains. Link them through batch ID, epoch and input/output belief revisions, and store the actual committed posterior. [Audit 01 R08](01_state_estimation.md) independently reproduces the logger's distinct-batch suppression at equal receipt time; receipt time must never substitute for identity. Receipt sample spacing also cannot bound a fixed unknown transport delay: adding the same delay to successive captures leaves their spacing unchanged. **September 7 cross-reference:** the additional delay and shutdown-tail reproductions are now saved in [audit 10](10_logging_and_event_accounting.md) and its linked evidence. They remain attributed to that investigation; they are not added to this historical 17-probe result, and no new run-level claim is made here.
