...
#: Exactly what --fast changes, and nothing else. Each entry is a cost knob, not a
#: geometry or camera-model knob, so the detector's operating point, the projection
#: and r_vis/r_miss are untouched.
#:
#: Shadows are deliberately NOT in here. Turning them off is the largest single
#: saving available, but shadows change what the cameras see, and this study
#: measures where a robot is easy or hard to detect. A development world whose
#: images differ from the evidence world in a detector-relevant way is not a
#: development world for this project. Contact rate is safe because it touches
#: only the physics contact report, never a rendered pixel.
FAST_PROFILE = {
...
<sdf version="1.8">
  <!-- warehouse_v2: three storage sections (north-south racking, racking turned
       ...
       artery.  Stock state {state}.{fast_note}
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fast", action="store_true",
                    help=("development derivative: contact sensors at 20 Hz. "
                          "Geometry, cameras, lighting and shadows are unchanged by "
                          "construction, so the rendered images are identical. "
