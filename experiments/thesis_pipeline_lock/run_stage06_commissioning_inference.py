#!/usr/bin/env python3
"""Run the frozen detector once over the authorized commissioning-fit images."""
...
def main() -> int:
    ...
    if protocol["inputs"]["authorized_role"] != "commissioning_fit":
        raise RuntimeError("Only commissioning_fit is authorized")
    ...
    with index_path.open(newline="", encoding="utf-8") as handle:
        authorized = [
            ...
            if row["dataset_split"] == protocol["inputs"]["authorized_role"]
        ]
    expected_views = int(protocol["inputs"]["authorized_views"])
    if len(authorized) != expected_views:
        raise RuntimeError(f"Expected {expected_views} commissioning views, found {len(authorized)}")
    ...
    with records_path.open("w", encoding="utf-8") as handle:
        for view_index, (row, result) in enumerate(
            ...
            if view_index % 500 == 0 or view_index == len(authorized):
                print(
                    f"commissioning inference {view_index}/{len(authorized)} views",
                    file=sys.stderr,
                    flush=True,
                )
    ...
    manifest = {
        ...
        "authorized_role": "commissioning_fit",
