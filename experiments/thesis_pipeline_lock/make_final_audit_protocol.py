...
opened, so it refuses to run while any locked input is missing.
    ...
    python3 experiments/thesis_pipeline_lock/make_final_audit_protocol.py
"""
...
HERE = REPO / "experiments/thesis_pipeline_lock"
V8 = "logs/thesis_final_pipeline_v1/recapture_v8_uniform"
LOCK = "experiments/thesis_pipeline_lock/reference_position_campaign_lock_v8_uniform.json"
...
def main() -> int:
    protocol = json.loads((HERE / "reference_final_audit_protocol.json").read_text())
    ...
    protocol["population"] = {
        ...
        "merge_loader": "experiments/warehouse_v2_sketches/combined_recapture_v8.py",
    }
    inputs = {
        "campaign_lock": LOCK,
        "gate_config": "config/commissioning_sensor_gate_v3.yaml",
        "label_protocol": "experiments/thesis_pipeline_lock/label_dataset_protocol.json",
        ...
        "implementation": "experiments/thesis_pipeline_lock/run_reference_final_audit.py",
    }
    protocol["locked_inputs"] = {key: {"path": rel, "sha256": sha(rel)}
                                 for key, rel in inputs.items()}
    protocol["amendment"] = "docs/THESIS_METHOD_CANONICAL_LOCK.md, Amendment 2026-09-23"
