...
lock is only written for a dataset that passed the presence check.
    ...
    THESIS_REFERENCE_DATASET=v8_uniform python3 experiments/thesis_pipeline_lock/relock_v8.py
"""
...
from experiments.warehouse_v2_sketches import combined_recapture_v8 as v8  # noqa: E402
...
def main() -> int:
    ...
    base = "experiments/thesis_pipeline_lock/reference_position_campaign_lock.json"
    ...
    lock.update({
        ...
        "campaign_root": R, "amendment": "docs/THESIS_METHOD_CANONICAL_LOCK.md, Amendment 2026-09-23",
        "derived_from": {"lock": base, "sha256": sha(base)},
    })
    lock["sampling"].update({
        "loader": "experiments/warehouse_v2_sketches/combined_recapture_v8.py",
        "positions": f"{R}/capture_positions_v8.csv", "positions_sha256": sha(f"{R}/capture_positions_v8.csv"),
        "topup_rule": "experiments/warehouse_v2_sketches/plan_uniform_topup.py",
    ...
    out = HERE / "reference_position_campaign_lock_v8_uniform.json"
