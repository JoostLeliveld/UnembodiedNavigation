...
assert set(sys.argv[1:])<={c['arm'] for c in selection['runs']},'Unknown arm'
...
for cfg in selection['runs']:
 ...
 if reason!='driver_finished' or json.loads((D/'summary.json').read_text())['status']!='belief_goal_reached':break
