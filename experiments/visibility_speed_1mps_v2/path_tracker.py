...
Uses pure pursuit with linear speed limited by the existing angular bound. The
...
def track(pose,states,controls,goal,lookahead=LOOKAHEAD):
    ...
    if abs(error)>.6:v=0.;w=np.clip(1.2*error,-PROFILE['max_angular_speed_rad_s'],PROFILE['max_angular_speed_rad_s'])
    else:
        curvature=2*np.sin(error)/max(distance,.05)
        if abs(curvature)>1e-12:v=min(v,PROFILE['max_angular_speed_rad_s']/abs(curvature))
        w=curvature*v
