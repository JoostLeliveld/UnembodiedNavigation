#!/usr/bin/env python3
"""Fail-closed validation for the polyline (v2) commissioning campaign.

Same integrity rules as v1 -- hashed artifacts, locked 1.0 m/s cruise, sealed audit
partition -- with two generalisations: a route is a polyline of any length, and the drive
count is whatever the protocol declares rather than a fixed 24.
"""
...
def validate(protocol_path: Path = HERE / "campaign_v2.yaml") -> dict[str, Any]:
    protocol_path = protocol_path.resolve()
    protocol = yaml.safe_load(protocol_path.read_text(encoding="utf-8"))
    if protocol.get("schema_version") != "reference_controlled_commissioning.v2":
        raise ProtocolError("unexpected commissioning schema")
    if protocol.get("status") not in {
        "polyline_campaign_pending_preflight",
    ...
    for route_id, route in protocol["routes"].items():
        ...
        if len(points) < 2:
            raise ProtocolError(f"{route_id}: a route needs at least two points")
        ...
        for index, (a, b) in enumerate(zip(points[:-1], points[1:])):
            if math.dist(a, b) < 1e-6:
                raise ProtocolError(f"{route_id}: leg {index} has zero length")
        # A path follower cannot execute a 180 degree reversal without stopping, so reject
        # any corner that doubles back along the leg it arrived on.
        for index in range(1, len(points) - 1):
            before = points[index - 1]
            corner = points[index]
            after = points[index + 1]
            incoming = _route_heading([before, corner])
            outgoing = _route_heading([corner, after])
            if abs(_angle_difference(outgoing, incoming)) > math.radians(170.0):
                raise ProtocolError(
                    f"{route_id}: waypoint {index} reverses the drive by more than 170 degrees"
                )
        heading = _route_heading(points[:2])
        clearances = []
        for a, b in zip(points[:-1], points[1:]):
            leg_heading = _route_heading([a, b])
            for x, y in sample_polyline(
                [a, b], maximum_step_m=float(geometry["route_sample_step_m"])
            ):
                clearances.append(body.clearance([float(x), float(y), leg_heading]))
        # A corner is rounded, so the body sweeps both headings there; check each corner
        # against the heading it arrives on and the one it leaves on.
        for index in range(1, len(points) - 1):
            for corner_heading in (_route_heading([points[index - 1], points[index]]),
                                   _route_heading([points[index], points[index + 1]])):
                clearances.append(body.clearance(
                    [points[index][0], points[index][1], corner_heading]))
        ...
        anchor_clearances = []
        for endpoint_index, base_heading in (
            (0, heading), (-1, _route_heading(points[-2:]) + math.pi)
        ):
        ...
        # Only directions the campaign actually drives are checked.  A closed loop has no
        # meaningful reverse start yaw, and declaring one would be a task nothing runs.
        driven = {drive["direction"] for drive in protocol["drives"]
                  if drive["route"] == route_id}
        for direction, task_key in (("forward", "forward_task"), ("reverse", "reverse_task")):
            if direction not in driven:
                continue
            task_name = route[task_key]
            if task_name not in tasks:
                raise ProtocolError(f"{route_id}: missing task {task_name}")
            directed = list(points) if direction == "forward" else list(reversed(points))
            ...
            expected_yaw = _route_heading(directed[:2])
    ...
    if len(drives) != declared:
        raise ProtocolError(f"expected {declared} drives, got {len(drives)}")
    ...
    if sorted(orders) != list(range(1, declared + 1)):
        raise ProtocolError(f"drive order must be exactly 1..{declared}")
    ...
    counts: dict[str, int] = {}
    per_partition = declared // len(expected_partitions)
    if declared % len(expected_partitions):
        raise ProtocolError("drive count must divide evenly across the three partitions")
    for partition in sorted(expected_partitions):
        members = [drive for drive in drives if drive["partition"] == partition]
        if len(members) != per_partition:
            raise ProtocolError(
                f"{partition}: expected {per_partition} drives, got {len(members)}"
            )
        counts[partition] = len(members)
    # No route-direction variant may appear in more than one partition: a lap used to fit
    # must never also be the held-out or audit evidence.
    owner: dict[tuple[str, str], str] = {}
    for drive in drives:
        variant = (drive["route"], drive["direction"])
        if variant not in route_variants:
            raise ProtocolError(f"unknown route-direction variant {variant}")
        if owner.setdefault(variant, drive["partition"]) != drive["partition"]:
            raise ProtocolError(
                f"{variant} appears in both {owner[variant]} and {drive['partition']}"
            )
    for drive in drives:
    ...
    return {
        "schema": "reference_controlled_commissioning_preflight.v2",
