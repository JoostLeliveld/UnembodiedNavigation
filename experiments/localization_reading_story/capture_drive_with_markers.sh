#!/usr/bin/env bash
# Capture ONE single-camera drive in the AWS warehouse WITH THE MARKER DISKS ON.
#
# Why this exists, when `experiments/filter_notebook/capture_aws_notebook_dataset.sh`
# already captures drives: those were recorded with `show_pose_markers` false, so their
# frames contain no marker disks and the keypoint reading cannot be scored on them. The
# question this folder is asked -- what does the filter do over TIME with each reading --
# needs both readings on the SAME motion, which needs one capture with the disks rendered.
#
# Everything else is that script, unchanged: same world, same route file, same detector,
# same recorders, same guards. The two differences are `show_pose_markers:=true` and
# `VIEW_EVERY=1`, because every saved frame is a sighting the keypoint model can be run on
# offline.
#
# Caveat carried into every number from this capture: the disks sit on top of the robot, so
# they move the detected box's TOP edge. The box bottom -- the edge the deployed reading
# uses -- is not touched, but the box is not bit-identical to one drawn on a bare robot.
#
#   ROUTE=aisle_east_north bash experiments/localization_reading_story/capture_drive_with_markers.sh
#
...
# Routes come from the filter_notebook study file by default; a longer route of
# this folder's own lives in config/long_route_study.yaml. Override with STUDY=.
STUDY="${STUDY:-$REPO/experiments/filter_notebook/config/aws_study.yaml}"
...
RUN_TAG="${RUN_TAG:-markers_${ROUTE}_$(date +%Y%m%d_%H%M%S)}"
OUT="$REPO/logs/studies/localization_reading_story/drives/$RUN_TAG"
...
echo "== launching the AWS world (headless), one camera, MARKER DISKS ON, spawn at the route start"
...
Path(out, "raw", "capture_manifest.json").write_text(json.dumps({
    ...
    "show_pose_markers": True,
    "purpose": ("both readings scored offline on the SAME frames: the box bottom through "
                "the frozen detector, the marked point through yolo_pose_aws_v4"),
...
python3 scripts/reliability/record_multicamera_views.py \
  ...
  --every "${VIEW_EVERY:-1}" --duration-s "$RECORD_S" \
  >"$LOGS/views.log" 2>&1 &
VIEWS=$!
