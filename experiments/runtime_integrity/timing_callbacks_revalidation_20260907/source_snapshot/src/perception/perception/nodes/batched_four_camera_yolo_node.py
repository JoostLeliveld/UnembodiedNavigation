...
class BatchedFourCameraYoloNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.batch_outcome_publisher = self.create_publisher(
            String, "/perception/camera_batch_outcome", 100
        )
    ...
    def _publish_batch_outcome(self, event) -> None:
        payload = dict(event, schema_version="camera_batch_outcome.v1", stage="detector",
            ...
            try:
                ...
                if cycle is not None:
                    self._publish_batch_outcome(dict(
                        source_batch_id=cycle[0], invocation_id=f"{cycle[0]}/chunk/{start // chunk}",
                        camera_ids=cycle[1][start:start + len(group)], status="inference_error",
                        inference_start_stamp_s=started_stamp, reason=str(exc)))
                raise
            if cycle is not None:
                self._publish_batch_outcome(dict(
                    source_batch_id=cycle[0], invocation_id=f"{cycle[0]}/chunk/{start // chunk}",
                    ...
                    inference_wall_ms=(time.perf_counter() - started_wall) * 1000.0))
    ...
    def _process_frames(self, batch: tuple[PendingFrame, ...]) -> None:
        ...
        try:
            ...
            self._publish_batch_outcome(dict(source_batch_id=source_batch_id,
                                             status="aborted", reason=str(exc), camera_ids=members))
            raise
        finally:
