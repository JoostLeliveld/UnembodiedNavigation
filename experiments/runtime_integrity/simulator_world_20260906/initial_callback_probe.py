...
def load_class(rel, class_name, keep_methods=None):
    path=ROOT/rel; tree=ast.parse(path.read_text())
...
def main():
    ...
    for _ in range(3):n._publish_latest()
    n.now_s=900.;n._publish_latest()
    m=Clock();m.clock=stamp(1.);n._clock_cb(m);n._publish_latest()
    outputs=[p.clock.sec+p.clock.nanosec*1e-9 for p in n._pub.messages]
    assert outputs==[100.,100.,100.,100.,1.]
    results['clock_republishes_after_source_loss_and_forwards_rewind']={'outputs_s':outputs,'new_upstream_samples':2,'wall_now_s':n.now_s}
    ...
    sources={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [*ROOT.glob('src/sim/sim/*py'),ROOT/'src/experiments/experiments/nodes/experiment_logger.py']}
    ...
    (HERE/'callback_results.json').write_text(json.dumps(result,indent=2)+'\n')
