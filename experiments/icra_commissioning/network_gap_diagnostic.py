...
def main(selection, arm, out):
    ...
    # Two seconds of published-belief context show the accepted update's effect.
    mask=nav.aligned.landed_mask(belief['stamp'])&(belief['stamp']>=lo)&(belief['stamp']<=hi+2.)
    ...
    nav.style();fig,axes=nav.plt.subplots(2,1,figsize=(9,6.1),sharex=True)
    fig.subplots_adjust(left=.18,right=.985,top=.84,bottom=.17,hspace=.12)
    ...
    fig.legend(handles,labels,loc='lower center',ncols=2,fontsize=8,frameon=False,
    ...
    axes[1].set(xlabel='Time since last accepted correction [s, simulation]',
                 ylabel='Belief position error [cm]',xlim=(-1,hi-lo+2.))
    axes[1].axvline(hi-lo,color='#207b70',ls='--',lw=1.)
    for ax in axes:ax.axvspan(hi-lo,hi-lo+2.,color='#e9eef0',zorder=0)
