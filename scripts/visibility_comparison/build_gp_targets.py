...
def _aggregate_rows_by_xy(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        bucket = grouped.setdefault(
            key,
            {
                ...
                'y': float(y),
                'count': 0,
                'values': {name: [] for name in value_keys},
            },
        )
    ...
    for idx, key in enumerate(sorted(grouped.keys(), key=lambda item: (item[1], item[0]))):
        ...
        out_rows.append({
            ...
            'red_binary': '' if not values['red_binary'] else f"{float(np.mean(values['red_binary'])):.8f}",
            'red_area_corrected': '' if not values['red_area_corrected'] else f"{float(np.mean(values['red_area_corrected'])):.8f}",
            'yolo_binary': '' if not values['yolo_binary'] else f"{float(np.mean(values['yolo_binary'])):.8f}",
            'yolo_score_raw': '' if not values['yolo_score_raw'] else f"{float(np.mean(values['yolo_score_raw'])):.8f}",
            'yolo_raw_best_score': '' if not values['yolo_raw_best_score'] else f"{float(np.mean(values['yolo_raw_best_score'])):.8f}",
            'yolo_score_calibrated': '' if not values['yolo_score_calibrated'] else f"{float(np.mean(values['yolo_score_calibrated'])):.8f}",
            'oracle_visibility': '' if not values['oracle_visibility'] else f"{float(np.mean(values['oracle_visibility'])):.8f}",
        })
    return out_rows
...
def main() -> int:
    ...
    write_manifest(output_dir / 'target_manifest.json', {
        ...
        'notes': [
            ...
            'gp_targets_xy_aggregated.csv averages detector targets over all sampled headings at each (x, y) position for heading-marginal 2D GP fitting.',
        ],
    })
