...
# =============================================================================
# Dynamic-scenario variant: same warehouse, obstacles that appear at run time
# =============================================================================
# The flagship world above is frozen evaluation infrastructure. Studies of
# *environmental change* (a pallet is set down in an aisle, a forklift drives
# through) need obstacles that spawn, move, stop and disappear on command, so
# they get their own world name and their own spawnable model assets — both
# generated here, never hand-edited.
#
# The world itself is deliberately IDENTICAL to the flagship apart from its
# name: every aisle starts clear and every obstacle arrives through a runtime
# spawn event. That keeps a dynamic run directly comparable to a static one and
# keeps "clear aisle at t=0" true by construction rather than by convention.
#
# Boundary: obstacle poses, oracle depth and ray-cast visibility produced from
# these assets are EVALUATION-ONLY. Nothing under src/ may read them.

DYN_WORLD_FILE = "warehouse_full_4cam_dynamic.world.sdf"
DYN_OUT = REPO / "src/sim/gazebo_worlds/worlds" / DYN_WORLD_FILE
DYN_STAGE = REPO / "src/sim/gazebo_worlds/worlds/warehouse_full_4cam_dynamic.stage.json"
DYN_MODELS = REPO / "src/sim/models"

# Where scenario obstacles are allowed to appear. Chosen so camera A (south
# wall, west dock column) looks straight down the lane: an obstacle here sits on
# A's sight-line to the cells behind it while barely touching the east-column
# cameras. Asserted clear of every rack/pillar occluder below, so "clear aisle"
# is a checked property of the generated world and not a claim in a README.
DYN_STAGE_AISLE = {
    "name": "rack_aisle_W2",
    "xmin": -6.13, "xmax": -5.22,
    "ymin": -8.35, "ymax": 8.35,
    "note": "north-south aisle between rack columns W1 and W2, aligned with camera A",
}


def _dyn_obstacle_models() -> dict:
    """Catalogue of spawnable obstacles, as a single rigid link each.

    One link with several ``<collision>``/``<visual>`` children keeps a
    multi-part shape (a forklift) rigid without joints. ``<static>false</static>``
    is required for gz to accept pose commands and to publish the pose on
    ``/world/<world>/dynamic_pose/info``; ``<gravity>false</gravity>`` then stops
    the body from settling or drifting, so a commanded pose is held exactly and
    two runs of the same scenario see byte-identical geometry.
    """
    def part(name, size, pose, color, *, collide=True):
        sx, sy, sz = size
        geom = f"<geometry><box><size>{sx:.3f} {sy:.3f} {sz:.3f}</size></box></geometry>"
        px, py, pz = pose
        out = []
        if collide:
            out.append(f'<collision name="{name}_collision"><pose>{px:.3f} {py:.3f} {pz:.3f} 0 0 0</pose>{geom}</collision>')
        out.append(f'<visual name="{name}_visual"><pose>{px:.3f} {py:.3f} {pz:.3f} 0 0 0</pose>{geom}'
                   f'<material><ambient>{color}</ambient><diffuse>{color}</diffuse></material></visual>')
        return "".join(out)

    pallet_wood = "0.45 0.31 0.16 1"
    load_tan = "0.78 0.62 0.34 1"
    fork_orange = "0.94 0.42 0.05 1"
    fork_dark = "0.18 0.18 0.20 1"
    steel = "0.55 0.57 0.60 1"

    # Loaded euro-pallet: 1.20 x 0.80 footprint, 1.49 m overall. Tall enough
    # that its shadow on a 0.35 m target is metres long from a 6.1 m mount, so
    # the occlusion it creates is unambiguous rather than marginal.
    pallet_parts = "".join([
        part("base", (1.20, 0.80, 0.14), (0.0, 0.0, 0.07), pallet_wood),
        part("load", (1.06, 0.72, 1.35), (0.0, 0.0, 0.815), load_tan),
    ])
    pallet = f"""<?xml version="1.0" ?>
<sdf version="1.8">
  <!-- Generated by scripts/geometry_visibility/make_warehouse_full.py with the
       dynamic variant selected. Edit the generator, not this file. -->
  <model name="dyn_pallet_box">
    <static>false</static>
    <link name="body">
      <gravity>false</gravity>
      <inertial><mass>60.0</mass><inertia>
        <ixx>12.0</ixx><iyy>14.0</iyy><izz>8.0</izz><ixy>0</ixy><ixz>0</ixz><iyz>0</iyz>
      </inertia></inertial>
      {pallet_parts}
    </link>
  </model>
...
"""
    ...
    # Forklift silhouette: counterweight body, driver mast, two floor-level
    # forks. 2.09 m tall at the mast, so it blocks more than the pallet does.
    forklift_parts = "".join([
        part("body", (1.60, 0.98, 1.05), (-0.30, 0.0, 0.60), fork_orange),
        part("cab_mast", (0.14, 0.90, 2.09), (0.52, 0.0, 1.045), steel),
        part("overhead_guard", (1.10, 0.90, 0.08), (0.05, 0.0, 2.05), fork_dark),
        part("fork_left", (1.10, 0.14, 0.07), (1.12, -0.26, 0.06), fork_dark),
        part("fork_right", (1.10, 0.14, 0.07), (1.12, 0.26, 0.06), fork_dark),
        part("wheel_front", (0.32, 1.02, 0.32), (0.30, 0.0, 0.16), fork_dark),
        part("wheel_rear", (0.32, 0.80, 0.32), (-0.90, 0.0, 0.16), fork_dark),
    ])
    forklift = f"""<?xml version="1.0" ?>
<sdf version="1.8">
  <!-- Generated by scripts/geometry_visibility/make_warehouse_full.py with the
       dynamic variant selected. Edit the generator, not this file. -->
  <model name="dyn_forklift">
    <static>false</static>
    <link name="body">
      <gravity>false</gravity>
      <inertial><mass>2200.0</mass><inertia>
        <ixx>600.0</ixx><iyy>900.0</iyy><izz>800.0</izz><ixy>0</ixy><ixz>0</ixz><iyz>0</iyz>
      </inertia></inertial>
      {forklift_parts}
    </link>
  </model>
...
"""
    return {
        "dyn_pallet_box": {
            "sdf": pallet,
            "description": "loaded euro-pallet, 1.20 x 0.80 x 1.49 m",
            # axis-aligned extent about the model origin at yaw 0, metres
            "extent_xyz": [1.20, 0.80, 1.49],
            "origin_is_floor_contact": True,
        },
        "dyn_forklift": {
            "sdf": forklift,
            "description": "forklift silhouette, 2.72 x 1.02 x 2.09 m including forks",
            "extent_xyz": [2.72, 1.02, 2.09],
            "origin_is_floor_contact": True,
        },
    }


def _dyn_model_config(name: str, description: str) -> str:
    return f"""<?xml version="1.0"?>
<model>
  <name>{name}</name>
  <version>1.0</version>
  <sdf version="1.8">model.sdf</sdf>
  <description>{description}. Generated by
  scripts/geometry_visibility/make_warehouse_full.py --variant dynamic.</description>
</model>
"""


def _dyn_assert_stage_is_clear() -> None:
    """Fail generation if any static occluder intrudes on the scenario aisle."""
    ax0, ax1 = DYN_STAGE_AISLE["xmin"], DYN_STAGE_AISLE["xmax"]
    ay0, ay1 = DYN_STAGE_AISLE["ymin"], DYN_STAGE_AISLE["ymax"]
    clashes = []
    for name, cx, cy, sx, sy, _h in occluders:
        ox0, ox1 = cx - sx / 2.0, cx + sx / 2.0
        oy0, oy1 = cy - sy / 2.0, cy + sy / 2.0
        if ox0 < ax1 and ox1 > ax0 and oy0 < ay1 and oy1 > ay0:
            clashes.append(name)
    if clashes:
        raise RuntimeError(
            f"dynamic stage aisle {DYN_STAGE_AISLE['name']} is not clear; "
            f"occluders intrude: {clashes}"
        )


def _dyn_camera_records() -> list[dict]:
    """Camera intrinsics/extrinsics for the output contract.

    Intrinsics follow the pinhole the renderer uses; the look-at extrinsic is
    derived exactly as ``reliability.projection.camera_model_from_world`` derives
    it, so an oracle grid and a runtime projection share one camera geometry.
    """
    width, height, fov = 1280, 720, 1.5708
    ...
    for label, include, _model, pose, topic, _color, mount in CAMERA_ITEMS:
        ...
        records.append({
            ...
            "mount": mount,
            "rgb_topic": topic,
            "oracle_depth_topic": topic.replace("/image_raw", "/depth"),
            "intrinsics": {
                "img_width": width, "img_height": height, "fov_h_rad": fov,
                "fx": f, "fy": f, "cx": width / 2.0, "cy": height / 2.0,
            },
            "extrinsics": {
                "sdf_pose_xyz_rpy": list(pose),
                "cam_pos": [x, y, z],
                "look_at": look_at,
            },
        })
...
def _write_dynamic_variant() -> None:
    _dyn_assert_stage_is_clear()

    header = (
        "  <!-- warehouse_full_4cam_dynamic: the flagship four-camera warehouse, used\n"
        "       for scenarios where the environment CHANGES during a run. Structurally\n"
        "       identical to warehouse_full_4cam so runs stay comparable; every aisle\n"
        "       starts clear and obstacles arrive through runtime spawn events using the\n"
        "       generated dyn_* models under src/sim/models/.\n"
        "       Generated by scripts/geometry_visibility/make_warehouse_full.py with\n"
        "       the dynamic variant selected - edit the generator, not this file. -->\n"
    )
    dyn = sdf.replace(
        '  <world name="warehouse_full_4cam">',
        header + '  <world name="warehouse_full_4cam_dynamic">',
        1,
    )
    if 'name="warehouse_full_4cam_dynamic"' not in dyn:
        raise RuntimeError("could not rename world for the dynamic variant")
    DYN_OUT.write_text(dyn)

    catalogue = _dyn_obstacle_models()
    for name, spec in catalogue.items():
        model_dir = DYN_MODELS / name
        model_dir.mkdir(parents=True, exist_ok=True)
        (model_dir / "model.sdf").write_text(spec["sdf"])
        (model_dir / "model.config").write_text(_dyn_model_config(name, spec["description"]))

    stage = {
        "generated_by": "scripts/geometry_visibility/make_warehouse_full.py --variant dynamic",
        "world_file": DYN_WORLD_FILE,
        "world_name": "warehouse_full_4cam_dynamic",
        "base_world_file": WORLD_FILE,
        "site_bounds": {"xmin": SITE_X0, "xmax": SITE_X1, "ymin": SITE_Y0, "ymax": SITE_Y1},
        "scenario_stage_aisle": DYN_STAGE_AISLE,
        "cameras": _dyn_camera_records(),
        "obstacle_catalogue": [
            {
                "model_name": name,
                "model_uri": f"model://{name}",
                "model_sdf": f"src/sim/models/{name}/model.sdf",
                "description": spec["description"],
                "extent_xyz": spec["extent_xyz"],
                "origin_is_floor_contact": spec["origin_is_floor_contact"],
            }
            for name, spec in catalogue.items()
        ],
        "static_occluder_models": ["warehouse_walls", "warehouse_rack_occluders"],
        "evaluation_only_note": (
            "Obstacle poses, oracle depth and ray-cast visibility derived from this "
            "world are EVALUATION-ONLY. No module under src/ may read them."
        ),
    }
    DYN_STAGE.write_text(json.dumps(stage, indent=2) + "\n")

    print(f"wrote {DYN_OUT}")
    print(f"wrote {DYN_STAGE}")
    for name in catalogue:
        print(f"wrote {DYN_MODELS / name}/model.sdf + model.config")
    print(f"  scenario stage aisle: {DYN_STAGE_AISLE['name']} "
          f"x[{DYN_STAGE_AISLE['xmin']:+.2f},{DYN_STAGE_AISLE['xmax']:+.2f}] "
          f"y[{DYN_STAGE_AISLE['ymin']:+.2f},{DYN_STAGE_AISLE['ymax']:+.2f}] (verified clear)")


def _write_static_variant() -> None:
    OUT.write_text(sdf)
...
_parser.add_argument(
    "--variant", choices=("static", "dynamic", "both"), default="static",
    help="static (default) = the frozen flagship world and its layout artifacts; "
         "dynamic = the change-during-a-run twin plus its spawnable obstacle models; "
         "both = write everything.",
)
_args = _parser.parse_args()

if _args.variant in ("static", "both"):
    _write_static_variant()
if _args.variant in ("dynamic", "both"):
    _write_dynamic_variant()
