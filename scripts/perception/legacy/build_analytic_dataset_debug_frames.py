#!/usr/bin/env python3
"""Archived YOLO dataset builder based on recorded detector frames.
...
launch log, then for every frame look up the robot's TRUE (x,y) at that image
stamp and project the robot's 3D extent (a ~0.14 m cylinder, 0.19 m tall).

This utility is retained only for reproducing historical artifacts. The
maintained dataset path is scripts/perception/capture_yolo_dataset.py.
...
"""
import sys, glob, csv, re, math, random, shutil, hashlib
...
MAX_DUPLICATE_IMAGE_FRACTION = 0.20
...
def load_poses(perc):
    exp = Path(perc).with_name("experiment.csv")
    if exp.exists():
        rows = list(csv.reader(open(exp))); h = rows[0]; ci = {n: i for i, n in enumerate(h)}
        st, tx, ty = [], [], []
        for r in rows[1:]:
            try:
                if float(r[ci["truth_available"]]) <= 0.0:
                    continue
                st.append(float(r[ci["truth_stamp"]]))
                tx.append(float(r[ci["truth_x"]]))
                ty.append(float(r[ci["truth_y"]]))
            except Exception:
                pass
    ...
    for r in rows[1:]:
        try:
            st.append(float(r[ci["diag_stamp"]]))
...
def assert_raw_frames_not_duplicate_heavy(frames):
    hashes = Counter()
    for fr in frames:
        hashes[hashlib.sha1(Path(fr).read_bytes()).hexdigest()] += 1
    duplicate_images = sum(count for count in hashes.values() if count > 1)
    duplicate_fraction = duplicate_images / max(len(frames), 1)
    print(
        f"raw exact-duplicate images: {duplicate_images}/{len(frames)} "
        f"({100.0 * duplicate_fraction:.1f}%), unique_hashes={len(hashes)}"
    )
    if duplicate_fraction > MAX_DUPLICATE_IMAGE_FRACTION:
        largest = ", ".join(f"{h[:10]}:{n}" for h, n in hashes.most_common(5))
        raise RuntimeError(
            "Refusing to build analytic YOLO dataset from repeated raw frames "
            f"({100.0 * duplicate_fraction:.1f}% duplicates; max allowed "
            f"{100.0 * MAX_DUPLICATE_IMAGE_FRACTION:.1f}%). "
            f"Largest groups: {largest}. Recapture a balanced dataset instead."
        )
...
def main():
    ...
    assert_raw_frames_not_duplicate_heavy(frames)
    if OUT.exists():
        shutil.rmtree(OUT)
