...
#: Development-only render/physics relief. It is a GENERATOR FLAG rather than a
#: hand-maintained second world file, because the previous derivative
#: (warehouse_v2_low_cpu.world.sdf, retired 2026-09-18 in 794f5920) silently
#: drifted from its parent by one obstacle while documenting itself as identical.
#: Deriving it here means the geometry CANNOT differ: every box, include, pose and
#: camera below is shared code, and only the fields listed in FAST_PROFILE change.
FAST = False

#: Include the two offline figure-capture cameras. Off by default: they render
#: every cycle and no runtime consumer reads them.
FIGURE_CAMERAS = False

#: THIS IS THE FASTEST SETTING AVAILABLE. Everything that can be made cheaper
#: without changing a rendered pixel is already applied, here and in the camera
#: models, and it is applied to the CANONICAL world too -- there is no faster
#: variant being held back, and nothing further to turn on:
#:
#:   * depth and segmentation sensors: REMOVED from every camera model. They
#:     rendered and published every cycle despite always_on=0 and no runtime
#:     consumer read them. Five cameras drove 20 render passes to use 5.
#:   * the two figure-capture cameras: removed from the worlds, opt-in behind
#:     --figure-cameras. One of them rendered 1600x1200 every cycle for stills
#:     that are captured offline.
#:   * contact sensors: 60 -> 20 Hz under --fast (below).
#:
#: The only remaining lever is shadows, and it is deliberately NOT here. Turning
#: them off is the largest single saving available, but shadows change what the
#: cameras see, and this study measures where a robot is easy or hard to detect.
#: A development world whose images differ from the evidence world in a
#: detector-relevant way is not a development world for this project. Contact
#: rate is safe precisely because it touches the physics contact report and
#: never a rendered pixel.
#:
#: So --fast is a small, pixel-identical delta on top of an already-pruned world.
#: Measured effect on the camera rate: NOT SEPARATED from run-to-run noise. Use
#: scripts/performance/benchmark_camera_rate.py before claiming any speedup.
FAST_PROFILE = {
    "contact_update_rate_hz": 20,      # 60 Hz -> 20 Hz contact sensors
}
...
def box_link(name, cx, cy, z, sx, sy, sz, colour, *, collide=True, contact=True,
    ...
    if collide:
        ...
        if contact:
            contact_hz = FAST_PROFILE["contact_update_rate_hz"] if FAST else 60
            p.append('<sensor name="contact" type="contact"><contact><collision>collision'
                     f'</collision></contact><update_rate>{contact_hz}</update_rate></sensor>')
    if visual:
...
def build_world(state: str = "A") -> str:
    ...
    # The two figure cameras are OPT-IN. Both are always_on=1 and render every
    # cycle -- one of them at 1600x1200 -- and nothing at navigation runtime reads
    # them: they exist to capture stills for figures, which is a separate offline
    # step. Leaving them in the world spent render budget on screenshots nobody
    # was taking. Pass --figure-cameras when capturing a figure frame.
    figure_cameras = "" if not FIGURE_CAMERAS else (
        ...
        "<pose>0.00 0.00 19.00 0 1.5708 1.5708</pose></include>\n"
    ...
    pruned_note = (
        "\n       RENDER COST: already minimal. The cameras carry the RGB sensor only"
        "\n       (depth and segmentation were removed: they rendered every cycle and no"
        "\n       runtime consumer read them), and the two offline figure-capture cameras"
        "\n       are not included; regenerate with the figure-cameras flag for stills."
        "\n       Shadows and lighting are intentionally UNCHANGED, because they change"
...
<sdf version="1.8">
  <!-- warehouse_v2: three storage sections (north-south racking, racking turned
       ...
       artery.  Stock state {state}.{pruned_note}{fast_note}
       Generated by world/make_world.py - edit the
...
if __name__ == "__main__":
    ...
    ap.add_argument("--figure-cameras", action="store_true",
                    help=("include the overview and plan-view cameras used to "
                          "capture figure stills. They render every cycle and no "
                          "runtime consumer reads them, so they are off by default."))
    ap.add_argument("--fast", action="store_true",
                    help=("FASTEST SETTING: contact sensors at 20 Hz on top of the "
                          "already-pruned canonical world. Geometry, cameras, "
                          "lighting and shadows are unchanged by construction, so "
                          "the rendered images are identical to the evidence world. "
                          "Requires an explicit --out so it cannot overwrite the "
                          "canonical world."))
    ap.add_argument(
        "--out",
        default=None,
    ...
    if a.fast:
        FAST = True
        if not a.out:
            raise SystemExit("--fast is a development world and requires an explicit --out")
    p = pathlib.Path(a.out) if a.out else WORLD_DIR / WORLD_FILES[a.state]
    world = build_world(a.state)
    # "--" is illegal inside an XML comment, and this header quotes flag names.
    # A malformed world reached a commit once because it was written after the
    # last parse; check every time rather than trusting review.
    for comment in re.findall(r"<!--(.*?)-->", world, re.S):
        if "--" in comment:
            raise SystemExit("generated header contains '--' inside an XML comment: "
                             + comment.strip()[:120])
    xml.dom.minidom.parseString(world)
    p.write_text(world)
    shown = p.resolve()
    try:
    ...
    print(f"wrote {shown}  ({p.stat().st_size/1024:.0f} kB)"
          ...
          + ("   [FAST: contact 20 Hz; rendering identical]" if a.fast else ""))
...
def world_profile() -> str:
    ...
    investigation_focus: >-
      ...
      Generated by world/make_world.py.
