...
def costmap_connectivity(costmap_matrix, connectivity_threshold = 0.0, diagonal_connectivity=False, edge_weight='mean'):
    ...
    for delta_row, delta_col in costmap_connectivity:
        ...
        start_row_indices, start_col_indices =  np.meshgrid(np.arange(max(0, -delta_row), min(number_of_rows, number_of_rows-delta_row)), 
                                                            np.arange(max(0, -delta_col), min(number_of_cols, number_of_cols-delta_col)))
        end_row_indices, end_col_indices = np.meshgrid(np.arange(max(0, delta_row), min(number_of_rows, number_of_rows+delta_row)), 
                                                       np.arange(max(0, delta_col), min(number_of_cols, number_of_cols+delta_col)))
        
        start_flat_indices = np.ravel_multi_index((start_row_indices.flatten(),  start_col_indices.flatten()), dims=(number_of_rows, number_of_cols))
        end_flat_indices = np.ravel_multi_index((end_row_indices.flatten(),  end_col_indices.flatten()), dims=(number_of_rows, number_of_cols))
